...
salida <-mmds(active, pc = 3, group.file = NULL)
 ...
 write.csv(x = salida$coord, file = "SABRE_PC3.csv", row.names = TRUE)
write.csv(x = salida$source, file = "SABRE_socurce.csv", row.names = TRUE)
...
cooord <- salida$coord
K <- kmeans.run(cooord, nb.clus = 3, nb.run = 1000, iter.max = 10000,method = "euclidean")
K

##########################GRAFICAR GGPLOT
library(ggplot2) yggrepel

filePC<-read.csv("MDScompletoEGL3.csv") # checar que el archivo este separado por comas si no no corre
ggplot(filePC,aes(x = x, y = y)) + geom_point(aes(color = factor(gear)))
